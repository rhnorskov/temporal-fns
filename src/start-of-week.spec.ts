...
describe("startOfWeek", () => {
	...
	it("respects weekStartsOn option (Sunday start)", () => {
		const date = Temporal.PlainDate.from("2024-02-21"); // Wed
		const result = startOfWeek(date, { weekStartsOn: 7 });

		expect(result.toString()).toBe("2024-02-18"); // Sun
	});

	it("returns the same day when weekStartsOn matches dayOfWeek", () => {
		const date = Temporal.PlainDate.from("2024-02-18"); // Sun
		const result = startOfWeek(date, { weekStartsOn: 7 });

		expect(result.toString()).toBe("2024-02-18");
	});
});
